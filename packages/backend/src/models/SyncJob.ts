...
syncJobSchema.pre('save', function (next) {
  ...
  if (this.performance.startedAt && this.performance.completedAt) {
    this.performance.duration =
      this.performance.completedAt.getTime() -
      this.performance.startedAt.getTime();

    if (this.performance.duration > 0 && this.processedItems > 0) {
      this.performance.itemsPerSecond =
        Math.round(
          (this.processedItems / this.performance.duration) * 1000 * 100
        ) / 100;
      this.performance.averageItemTime = Math.round(
        this.performance.duration / this.processedItems
      );
    }
  }
...
syncJobSchema.methods.addError = async function (
  error: ISyncError
): Promise<void> {
