...
async function testShopifyAPI() {
  ...
  if (!shopDomain || !accessToken) {
    log(
      '❌ SHOPIFY_SHOP_DOMAIN 또는 SHOPIFY_ACCESS_TOKEN이 설정되지 않았습니다.',
      colors.red
    );
  ...
  try {
    ...
    log(
      `✅ REST API 성공! 상품 수: ${restResponse.data.products.length}`,
      colors.green
    );
    ...
    if (restResponse.data.products.length > 0) {
      ...
      if (!variant.sku) {
        ...
        log(
          `Shopify 관리자에서 상품 편집 > Inventory > SKU 필드에 값을 입력하세요.`,
          colors.yellow
        );
      }
    } else {
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        log(
          `\n💡 인증 실패: Access Token이 잘못되었거나 만료되었습니다.`,
          colors.yellow
        );
      } else if (error.response.status === 404) {
        log(
          `\n💡 API 버전이 잘못되었을 수 있습니다. 현재: ${apiVersion}`,
          colors.yellow
        );
      }
    }
  ...
  for (const testSku of testSkus) {
    try {
      ...
      if (variants.length > 0) {
        log(
          `✅ SKU '${testSku}' 찾음! 결과 수: ${variants.length}`,
          colors.green
        );
    ...
    products.forEach((productEdge: any) => {
      ...
      product.variants.edges.forEach((variantEdge: any) => {
        ...
        if (variant.sku) {
          ...
          log(
            `  SKU: ${variant.sku} - ${product.title} / ${variant.title}${vendorTag}`,
  ...
  log(
    '5. ✅ SHOPIFY_API_VERSION이 2025-04로 설정되어 있는지 확인',
    colors.yellow
  );
...
async function testShopifyAPI() {
  ...
  try {
    ...
    if (variants.length > 0) {
      log(
        `✅ SKU 검색 성공! 찾은 variant 수: ${variants.length}`,
        colors.green
      );
      ...
      if (!skuFound) {
        ...
        log(
          `💡 Shopify 관리자 페이지에서 상품에 SKU를 추가하세요.`,
          colors.yellow
        );
      }
    }
  ...
  log(
    '3. SHOPIFY_SHOP_DOMAIN이 올바른지 확인 (예: myshop.myshopify.com)',
    colors.yellow
  );
