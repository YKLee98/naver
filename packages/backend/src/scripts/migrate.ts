...
class MigrationRunner {
  private migrations: Migration[] = [
    {
      ...
      up: async () => {
        ...
        await ProductMapping.collection.createIndex({ syncStatus: 1, lastSyncedAt: -1 });
        
        // InventoryTransaction 인덱스
        await InventoryTransaction.collection.createIndex({ sku: 1, createdAt: -1 });
        await InventoryTransaction.collection.createIndex({ orderId: 1, orderLineItemId: 1 });
        
        // PriceHistory 인덱스
        await PriceHistory.collection.createIndex({ sku: 1, createdAt: -1 });
        
        // OrderSyncStatus 인덱스
        await OrderSyncStatus.collection.createIndex({ platform: 1, orderDate: -1 });
  ...
  async status(): Promise<void> {
    ...
    this.migrations.forEach(migration => {
      ...
      const executedAt = executed ? executed.executedAt.toISOString() : 'Not executed';
      
      logger.info(`[${status}] ${migration.version} - ${migration.name} (${executedAt})`);
    });
  }
}
