...
async function testNaverAPIDirect() {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      if (status === 200) {
        ...
        if (response.data) {
          log(
            `   응답: ${JSON.stringify(response.data).substring(0, 200)}`,
            colors.yellow
          );
        }
      }
  ...
  try {
    const sellerResponse = await axios.get(
      'https://api.commerce.naver.com/external/v1/seller-info',
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
    ...
    if (sellerResponse.status === 200) {
      ...
      if (sellerResponse.data?.sellerId) {
        ...
        log(
          `   스토어명: ${sellerResponse.data.storeName || 'N/A'}`,
          colors.green
        );
      }
    } else {
