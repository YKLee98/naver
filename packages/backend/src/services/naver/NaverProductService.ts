...
export interface NaverProductSearchOptions {
  ...
  searchType?: 'PRODUCT_NAME' | 'PRODUCT_TAG' | 'SELLER_MANAGEMENT_CODE' | 'PRODUCT_NO';
  page?: number;
  size?: number;
}
...
export class NaverProductService {
  ...
  async searchProductsBySellerManagementCode(sku: string): Promise<NaverProduct[]> {
    try {
      ...
      logger.error(`Error searching products by seller management code: ${sku}`, error);
   ...
   * 대량 재고 업데이트
   */
  async bulkUpdateStock(updates: Array<{ productId: string; quantity: number }>): Promise<any> {
    try {
   ...
   * 대량 가격 업데이트
   */
  async bulkUpdatePrices(updates: Array<{ productId: string; price: number }>): Promise<any> {
    try {
      const results = await Promise.allSettled(
        updates.map(update => 
