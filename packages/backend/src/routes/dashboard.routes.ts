...
export function setupDashboardRoutes(services: ServiceContainer): Router {
  ...
  if (services.dashboardController) {
    ...
    // Widgets
    router.get('/widgets', ctrl.getWidgets?.bind(ctrl) || defaultWidgetsHandler);
    router.get('/widgets/:widgetId', ctrl.getWidget?.bind(ctrl) || defaultWidgetHandler);
...
async function defaultStatisticsHandler(req: any, res: any) {
  try {
    const { ProductMapping, Activity, SyncJob } = await import('../models/index.js');
...
async function defaultStatisticsTypeHandler(req: any, res: any) {
  try {
    ...
    const { ProductMapping, Activity, SyncJob, InventoryTransaction } = await import('../models/index.js');
    ...
    switch (type) {
      ...
      case 'sync':
        const [totalSyncs, completedSyncs, failedSyncs, pendingSyncs] = await Promise.all([
          SyncJob.countDocuments(),
          SyncJob.countDocuments({ status: 'completed' }),
          SyncJob.countDocuments({ status: 'failed' }),
          SyncJob.countDocuments({ status: 'pending' })
        ]);
        data = { total: totalSyncs, completed: completedSyncs, failed: failedSyncs, pending: pendingSyncs };
        break;
