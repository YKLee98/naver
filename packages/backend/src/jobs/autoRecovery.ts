...
export class AutoRecoveryJob {
  ...
  private async recoverPendingMappings() {
    try {
      ...
      logger.info(
        `Found ${pendingMappings.length} pending mappings to recover`
      );
  ...
  private async recoverSingleMapping(mapping: any) {
    ...
    if (mapping.naverProductId === 'PENDING') {
      try {
        ...
        logger.warn(
          `Failed to search Naver product for ${mapping.sku}:`,
          error
        );
      }
    }
    ...
    if (
      ...
      try {
        ...
        if (response.data?.products?.edges?.length > 0) {
          ...
          if (variant) {
            ...
            mapping.shopifyInventoryItemId =
              variant.inventoryItem?.id?.split('/').pop() || null;
        ...
        logger.warn(
          `Failed to search Shopify product for ${mapping.sku}:`,
          error
        );
      }
    }
